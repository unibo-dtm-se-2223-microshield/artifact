/**
 * @file microshield_cobs.c
 * @brief Implementation of IEEE 802.3 CRC32 checksum and COBS serialization.
 */
...
/* Monotonic sequence counter for outbound telemetry frames */
static uint32_t g_sequence_counter = 0U;

/* Internal device identifier */
static uint16_t g_node_id = 0U;

/**
 * @brief Precomputed IEEE 802.3 standard polynomial lookup table (0xEDB88320).
 * Stored statically in Flash (.rodata), consuming 0 bytes of volatile RAM.
 */
...
size_t microshield_cobs_encode(const uint8_t *src, size_t length, uint8_t *dst) {
    ...
    dst[code_index] = code;
    dst[write_index++] = 0x00U; /* Trailing packet delimiter */
...
size_t microshield_cobs_decode(const uint8_t *src, size_t length, uint8_t *dst) {
    ...
    while (read_index < length) {
        ...
        if (code == 0U) {
            return 0U; /* Framing error: premature null byte */
        }

        for (uint8_t i = 1U; i < code; i++) {
            if (read_index >= length) {
                return 0U; /* Truncated buffer */
            }
            dst[write_index++] = src[read_index++];
        }

        if ((code < 0xFFU) && (read_index < length)) {
            dst[write_index++] = 0x00U;
        }
    }
...
uint16_t microshield_build_telemetry(microshield_verdict_t verdict, 
    ...
    if ((features == NULL) || (out_frame == NULL)) {
        return 0U;
    }

    out_frame->node_id = g_node_id;
    out_frame->rule_id = rule_id;
    out_frame->sequence_id = g_sequence_counter++;
    out_frame->verdict = (uint8_t)verdict;
    out_frame->split_feature = split_feature;
    out_frame->reserved = 0U;

    out_frame->features[0] = features->norm_length;
    out_frame->features[1] = features->delta_time_us;
    out_frame->features[2] = features->protocol_flags;
    out_frame->features[3] = features->byte_variance;

    /* Compute CRC32 over the payload (32 bytes total minus the 4-byte CRC field) */
    const size_t payload_len = sizeof(microshield_telemetry_t) - sizeof(uint32_t);
    out_frame->crc32 = microshield_crc32((const uint8_t *)out_frame, payload_len);

    return (uint16_t)sizeof(microshield_telemetry_t);
}
